...
public class MegaTronPropNet extends DiscountedRewardGamer {
		...
		while (!SystemCalls.passedTime(finishBy) && runs < maxSamples) {
			try {
				...
				if (count > 0) {
					...
					//	heuristicScoreCache.cache(state, (sumScore / count));
				}
			...
			if (theMachine.isTerminal(state)) {
				...
				for (Move move : myMoves) {
					...
					for (List<Move> jointMove : jointMoves) {
						...
						double score = getStateValue(next, finishBy, alpha, beta, depth + 1, maxDepth, nextFinalValue);
						if (score < 0) return -1;
